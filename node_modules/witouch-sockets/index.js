var RPISessionHolder, WEBSessionHolder;
var io;
...
exports.init = function (port, expressApp){
    ...
    io.of('feedStatus').on('connection',function(socket){
        ...
        //WEBSessionHolder.add(socket.id, socket.handshake.query.clientID);
        console.log(" /////////////////////////////");
        ...
        socket.on("disconnect",function(){
            //WEBSessionHolder.remove(socket.id);
	...
	io.of('rpi').on('connection', function (socket) {
		...
		socket.on("clientIdentification",function(data){
			...
			console.log("Client identification received");
			io.of('status').emit('rpiConnectionsUpdated',RPISessionHolder.connectedClients);
...
/*WEBSessionHolder = module.exports.webSessionHolder = {
    sessions: {},
    connectedClients : {},
    add: function(socketID,clientID) {
        WEBSessionHolder.sessions[socketID] = clientID;
        WEBSessionHolder.connectedClients[clientID] = true;
        console.log(WEBSessionHolder.sessions);
    },
    remove: function(socketID) {
        WEBSessionHolder.connectedClients[WEBSessionHolder.sessions[socketID]] = false;
        delete WEBSessionHolder.sessions[socketID];
        console.log(WEBSessionHolder.sessions);       
    },
    isOnline : function(clientID){
        var result = false;
        Object.keys(WEBSessionHolder.sessions).forEach(function(key,value) {
            if (WEBSessionHolder.sessions[key]+"" == clientID+""){
                result =  true;
            }
        });
    ...
    findSocketUsingClient : function (clientID){
        var result = false;
        Object.keys(WEBSessionHolder.sessions).forEach(function(key,value) {
            console.log(WEBSessionHolder.sessions[key]+""+" // "+clientID);
            if (WEBSessionHolder.sessions[key]+"" == clientID+""){
                console.log("FOUND !");
                result =  key+"";
            }
